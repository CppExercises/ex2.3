...
auto populate_with_sequence(C &ctnr, typename C::value_type start)
{
  ...
  */
  std::generate(ctnr.begin(), ctnr.end(), [n = start] () mutable { return n++; });
}
...
auto multiply_with_value(C &ctnr, typename C::value_type value)
{
  ...
  */
  std::for_each(ctnr.begin(), ctnr.end(), [value](auto &c){c *= value; });
}
...
auto count_fulfills_cond(C &ctnr, UnaryPredicate condition)
{
  ...
  return count;
  */
  return std::count_if(ctnr.begin(), ctnr.end(), condition);
}
...
auto first_n_equal(const C &a, const C &b, std::size_t n)
{
  ...
  */
  ...
  /*
  // Question a.begin() + n
  // Compiler said the error is about the operater +
  if (std::equal(a.begin(), a.begin() + n, b.begin()))
  {
    return true;
  }
  else
  ...
  */
  ...
  if (std::equal(a.begin(), iterator, b.begin()))
  {
    return true;
  }
  else
...
template <typename C> auto abssum_of_elements(C &ctnr)
{
  ...
  return sum;
  */
  std::for_each(ctnr.begin(), ctnr.end(), [](auto &c) {c = std::abs(c);});
  return std::accumulate(ctnr.begin(), ctnr.end(), 0);
}
...
template <typename C> auto print(std::ostream &os, const C &ctnr)
{
  ...
  */
  ...
  {
    typename C::const_iterator itr = ctnr.begin();
    os << "[" << *itr++;
    while (itr != ctnr.end())
    {
      os << ", " << *itr++;
